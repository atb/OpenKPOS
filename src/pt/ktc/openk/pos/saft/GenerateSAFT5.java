...
import oecd.standard.audit.file.tax.p.t1._0._401.util._401ResourceFactoryImpl;
import oecd.standard.audit.file.tax.p.t1._0._401.util._401ResourceImpl;
...
public class GenerateSAFT5 implements Runnable {
	...
	public GenerateSAFT5(Date d1, Date d2, String xmlFile, JTextArea output, AppView oApp, JLabel progress) {
		mDate1=d1;
		mDate2=d2;
		mXmlFile=xmlFile;
		jtxtOutput=output;
	...
	public void run() {
		...
		try {
			...
			Resource.Factory.Registry.INSTANCE.getExtensionToFactoryMap().put(StringConstants.getString(StringEnumerates.saft), new _401ResourceFactoryImpl());

			// Criar em memoria o modelo do SAFT
			// Primeiro o documentRoot
			oecd.standard.audit.file.tax.p.t1._0._401.DocumentRoot root= _401Factory.eINSTANCE.createDocumentRoot();

			// o AuditFile
			oecd.standard.audit.file.tax.p.t1._0._401.AuditFileType aft = _401Factory.eINSTANCE.createAuditFileType();
			...
			// o Header
			oecd.standard.audit.file.tax.p.t1._0._401.HeaderType ht=_401Factory.eINSTANCE.createHeaderType();
			...
			ht.setAuditFileVersion("1.04_01");   // NECESSARIO ALTERAR
			...
			ht.setTaxAccountingBasis(oecd.standard.audit.file.tax.p.t1._0._401.TaxAccountingBasisType.F);

			oecd.standard.audit.file.tax.p.t1._0._401.AddressStructurePT ad=_401Factory.eINSTANCE.createAddressStructurePT();
			...
			oecd.standard.audit.file.tax.p.t1._0._401.SourceDocumentsType sd=_401Factory.eINSTANCE.createSourceDocumentsType();

			aft.setSourceDocuments(sd);

			// acrescentar o masterFiles
			oecd.standard.audit.file.tax.p.t1._0._401.MasterFilesType mf = _401Factory.eINSTANCE.createMasterFilesType();

			// salesinvoices
			oecd.standard.audit.file.tax.p.t1._0._401.SalesInvoicesType si=_401Factory.eINSTANCE.createSalesInvoicesType();
			...
			oecd.standard.audit.file.tax.p.t1._0._401.CustomerType cs = _401Factory.eINSTANCE.createCustomerType();
			...
			oecd.standard.audit.file.tax.p.t1._0._401.AddressStructure add = _401Factory.eINSTANCE.createAddressStructure();
			...
			*/
			...
			for(Object f: list.list().toArray()) {
				...
				cs = _401Factory.eINSTANCE.createCustomerType();
				...
				add = _401Factory.eINSTANCE.createAddressStructure();
			...
			oecd.standard.audit.file.tax.p.t1._0._401.TaxTableType taxTable=_401Factory.eINSTANCE.createTaxTableType();
			...
			for(Object f: lTaxes.list().toArray()) {
				...
				oecd.standard.audit.file.tax.p.t1._0._401.TaxTableEntryType tax_saft=_401Factory.eINSTANCE.createTaxTableEntryType();
				...
				tax_saft.setTaxType(oecd.standard.audit.file.tax.p.t1._0._401.TaxTypeType.IVA);		// Isto nao devia estar hardcoded
			...
			for(Object f: lProducts.list().toArray()) {
				...
				oecd.standard.audit.file.tax.p.t1._0._401.ProductType pType=_401Factory.eINSTANCE.createProductType();
				...
				pType.setProductType(oecd.standard.audit.file.tax.p.t1._0._401.ProductTypeType.P);
				
				// (ATB) v1.4.01 - Product Type: Produto ou Servico... Isto n‹o existe na aplicacao. A rever no futuro. Podia ser feito atraves da categoria do produto na aplicacao
				pType.setProductType(ProductTypeType.P);
			...
			for(Object f: lTickets.list().toArray()) {
				...
				if ((ti.getTicketType()==TicketInfo.RECEIPT_NORMAL) || (ti.getTicketType()==TicketInfo.RECEIPT_REFUND) || (ti.getTicketType()==pt.ktc.openk.pos.aspects.TicketInfo.RECEIPT_INVOICE)) {
					...
					oecd.standard.audit.file.tax.p.t1._0._401.InvoiceType invoice=_401Factory.eINSTANCE.createInvoiceType();

					invoice.setInvoiceNo(ti.getDocumentNo());
					
					// (ATB) v1.4.01 - Novo campo com codigo unico de documento da AT. Para ja preencher com zero
					invoice.setATCUD("0");
					...
					oecd.standard.audit.file.tax.p.t1._0._401.DocumentStatusType3 docStatus=_401Factory.eINSTANCE.createDocumentStatusType3();
					
					docStatus.setInvoiceStatus(oecd.standard.audit.file.tax.p.t1._0._401.InvoiceStatusType.N);		// Documento "Normal"
					...
					docStatus.setSourceBilling(oecd.standard.audit.file.tax.p.t1._0._401.SAFTPTSourceBilling.P);
					// -------
					...
					case TicketInfo.RECEIPT_NORMAL:
						...
						invoice.setInvoiceType(oecd.standard.audit.file.tax.p.t1._0._401.InvoiceTypeType.FS);
						break;
					case pt.ktc.openk.pos.aspects.TicketInfo.RECEIPT_INVOICE:
						// Novo documento do tipo Fatura 
						invoice.setInvoiceType(oecd.standard.audit.file.tax.p.t1._0._401.InvoiceTypeType.FT);
						break;
					case TicketInfo.RECEIPT_REFUND:
						// TD = talao de devolucao ->> DEVE PASSAR A NOTA DE CREDITO 'NC'
						invoice.setInvoiceType(oecd.standard.audit.file.tax.p.t1._0._401.InvoiceTypeType.NC);
						break;
					...
					oecd.standard.audit.file.tax.p.t1._0._401.SpecialRegimes specialRegime=_401Factory.eINSTANCE.createSpecialRegimes();
					...
					{
						oecd.standard.audit.file.tax.p.t1._0._401.LineType3 line=_401Factory.eINSTANCE.createLineType3();
						...
						if (tci.getMultiply()<0)
						{
							...
							oecd.standard.audit.file.tax.p.t1._0._401.References newRef=_401Factory.eINSTANCE.createReferences();
...
//							ref.setCreditNote(cn);
						...
						else
						{
							...
							if ((reference!=null) && (!reference.isEmpty())) {
								...
								// for esse o caso
								oecd.standard.audit.file.tax.p.t1._0._401.OrderReferences newRef=_401Factory.eINSTANCE.createOrderReferences();
						...
						oecd.standard.audit.file.tax.p.t1._0._401.Tax taxLine=_401Factory.eINSTANCE.createTax();
						...
						taxLine.setTaxType(oecd.standard.audit.file.tax.p.t1._0._401.TaxTypeType.IVA);
						
						double taxa2=tci.getTaxRate()*100;
						if (taxa==0) 
						{
							taxLine.setTaxPercentage(BigDecimal.valueOf(0));
						}
						else
						{
						...
						if (taxa==0) {
							...
							// (ATB) v1.4.01 - Agora e necessario adicionar o codigo do motivo de isencao
							// Para ja estamos a suportar hardcoded o caso do KTC Ensino: M07 Isento Artigo 9.o do CIVA (ou similar)
							line.setTaxExemptionCode("M07");
						}
					...
					oecd.standard.audit.file.tax.p.t1._0._401.DocumentTotalsType3 dt=_401Factory.eINSTANCE.createDocumentTotalsType3();
					//----------------
...
//					dt.setTaxPayable(totalVATCredit.subtract(totalVATDebit));
			...
			// salesinvoices
			oecd.standard.audit.file.tax.p.t1._0._401.WorkingDocumentsType wd=_401Factory.eINSTANCE.createWorkingDocumentsType();
			...
			for(Object f: lTickets.list().toArray()) {
				...
				if (ti.getTicketType()==pt.ktc.openk.pos.aspects.TicketInfo.RECEIPT_PREVIEW) {
					...
					oecd.standard.audit.file.tax.p.t1._0._401.WorkDocumentType workingDoc=_401Factory.eINSTANCE.createWorkDocumentType();

					workingDoc.setDocumentNumber(ti.getDocumentNo());
					
					// (ATB) v1.4.01 - Novo campo com codigo unico de documento da AT. Para ja preencher com zero
					workingDoc.setATCUD("0");
					...
					// Criar estrutura para estado do documento
					oecd.standard.audit.file.tax.p.t1._0._401.DocumentStatusType docStatus=_401Factory.eINSTANCE.createDocumentStatusType();
					...
					if ((lRelatedInvoices==null) || (lRelatedInvoices.list().isEmpty())) {
						...
						docStatus.setWorkStatus(oecd.standard.audit.file.tax.p.t1._0._401.WorkStatusType.N);		// Documento "Normal"

//						jtxtOutput.append("\n"+ti.getDocumentNo()+" -> X");
					}
					else {
						// Existem faturas relacionadas
						docStatus.setWorkStatus(oecd.standard.audit.file.tax.p.t1._0._401.WorkStatusType.F);		// Documento "Faturado"
...
//							jtxtOutput.append("\n"+ti.getDocumentNo()+" -> "+relatedTicket.getInvoiceNo());
					...
					//workingDoc.setWorkType(oecd.standard.audit.file.tax.p.t1._0._401.WorkTypeType.DC);
					// (ATB) O codigo para as consultas de mesa mudou na versao 1.4.01
					workingDoc.setWorkType(oecd.standard.audit.file.tax.p.t1._0._401.WorkTypeType.CM);
					...
					for (TicketLineInfo tci:ti.getLines())
					{
						oecd.standard.audit.file.tax.p.t1._0._401.LineType2 line=_401Factory.eINSTANCE.createLineType2();
					...
					// Acrescentar a estrutura com os totais do documento
					oecd.standard.audit.file.tax.p.t1._0._401.DocumentTotalsType dt=_401Factory.eINSTANCE.createDocumentTotalsType();
					//----------------
			...
			Resource resource2 = new _401ResourceImpl(URI.createURI(StringConstants.getString(StringEnumerates.file___)+mXmlFile));
